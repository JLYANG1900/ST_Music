...
注意：必须用<music>与</music>包裹这部分输出内容`;
    ...
    handleFileUpload(e) {
        ...
        this.player.playlist.push(...newTracks);
        this.renderPlaylist();

        if (typeof toastr !== "undefined") toastr.success(`已添加 ${newTracks.length} 首歌曲`);
    },
    ...
    renderPlaylist() {
        ...
        this.player.playlist.forEach((track, idx) => {
            ...
            item.className = "stm-track-item" + (this.player.currentTrackIndex === idx ? " active" : "");
            item.innerHTML = `
                <div class="stm-track-info">
                    <span class="stm-track-num">${this.player.currentTrackIndex === idx ? '<i class="fa-solid fa-volume-high"></i>' : String(idx + 1).padStart(2, '0')}</span>
                    <span class="stm-track-name">${track.name}</span>
                </div>
                <button class="stm-track-delete" title="删除"><i class="fa-solid fa-trash"></i></button>
            `;

            item.onclick = (e) => {
                if (!e.target.closest(".stm-track-delete")) {
                    this.playTrack(idx);
                }
            };

            item.querySelector(".stm-track-delete").onclick = (e) => {
                e.stopPropagation();
    ...
    playTrack(index) {
        ...
        const audioPlayer = document.getElementById("stm-audio-player");
        if (audioPlayer) {
            audioPlayer.src = this.player.playlist[index].url;
    ...
    togglePlay() {
        ...
        this.updatePlayIcon(this.player.isPlaying);
        this.updateDiscAnimation(this.player.isPlaying);
    },

    nextTrack() {
        if (this.player.playlist.length === 0 || this.player.currentTrackIndex === null) return;
        const nextIndex = (this.player.currentTrackIndex + 1) % this.player.playlist.length;
        this.playTrack(nextIndex);
    },

    prevTrack() {
        if (this.player.playlist.length === 0 || this.player.currentTrackIndex === null) return;
        const prevIndex = (this.player.currentTrackIndex - 1 + this.player.playlist.length) % this.player.playlist.length;
    ...
    deleteTrack(index) {
        ...
        if (this.player.currentTrackIndex === index) {
            const audioPlayer = document.getElementById("stm-audio-player");
        ...
        } else if (this.player.currentTrackIndex > index) {
    ...
    updatePlayIcon(isPlaying) {
        const icon = document.getElementById("stm-play-icon");
        if (icon) {
    ...
    updateDiscAnimation(isPlaying) {
        const disc = document.querySelector(".stm-disc-icon");
        if (disc) {
